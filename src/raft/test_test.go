...
// todo  raft_replication.go:116 rf.log[idx].Term != args.Term 写成了= 导致错误
...
// todo  raft_replication.go : 44 - 48 这个判断里忘记return 造成错误
func TestRejoinPartB(t *testing.T) {
	...
	/*
		cfg.one(101, servers, true) 这个函数调用的作用可能是尝试将指令 101 提交给一组服务器，
		并等待至少 servers 个服务器达成一致意见。
	*/
	// 检查当前集群中是否有且仅有一个领导者
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
